...
<body>
	<br>
	<p class="display-5 mb-4" style="font-size: 30px"> Lista resumida de espécies da resgate </p>
	...
	<table class="table">
		...
		$sql = "SELECT distinct(especie), familia, nome_popular, count(numero_ind) as total FROM resgate WHERE especie LIKE '%$searchTerm%' GROUP BY especie";
		...
		?>
			<tr>
				...
				<td>
					<a role="button" class="btn btn-outline-success btn-lg" href="imagem_resgate.php?sp=<?php echo $especie ?>&familia=<?php echo $familia ?>"><i class="fa-solid fa-image"></i></a></p>
				</td>
			<?php } ?>
