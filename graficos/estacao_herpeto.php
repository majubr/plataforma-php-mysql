...
$sql =  "SELECT estacao, count(estacao) as numero  FROM herpetofauna GROUP BY estacao";
...
while ($array = $stmt->fetch(PDO::FETCH_ASSOC)) {
 $estacao = $estacao . '"' . $array['estacao'] . '",';
	 $numero = $numero . '"' . $array['numero'] . '",';

	 $estacao = trim($estacao); #tira os espaços da variável
	 $numero = trim($numero);
...
$sql =  "SELECT estacao, count(estacao) as numero  FROM herpetofauna WHERE ano='2018' GROUP BY estacao";
$stmt = $conn->prepare($sql);
$stmt->execute();
...
$sql =  "SELECT estacao, count(estacao) as numero  FROM herpetofauna WHERE ano='2019' GROUP BY estacao";
$stmt = $conn->prepare($sql);
$stmt->execute();
...
function updateChartType4() {
	...
	<?php
$sql =  "SELECT estacao, count(estacao) as numero  FROM herpetofauna WHERE ano='2020' GROUP BY estacao";
$stmt = $conn->prepare($sql);
$stmt->execute();
...
if (document.getElementById('chartValor').value = '2020') 
  ...
  events: [],
	...
	function updateChartType5() {
			<?php
$sql =  "SELECT estacao, count(estacao) as numero  FROM herpetofauna WHERE ano='2021' GROUP BY estacao";
$stmt = $conn->prepare($sql);
$stmt->execute();
...
$sql =  "SELECT estacao, count(estacao) as numero  FROM herpetofauna WHERE ano='2022' GROUP BY estacao";
$stmt = $conn->prepare($sql);
$stmt->execute();
...
$sql =  "SELECT estacao, count(estacao) as numero  FROM herpetofauna WHERE ano='2023' GROUP BY estacao";
$stmt = $conn->prepare($sql);
$stmt->execute();
